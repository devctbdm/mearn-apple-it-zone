...
function parseSpecifications(raw: any): {
  keySpecs: KeySpec[];
  keyFeatures: KeyFeature[];
  ...
  // new structured format: { _keySpecs: {...}, _keyFeatures: {...}, _specGroups: {...} }
  if (data._keySpecs || data._keyFeatures || data._specGroups) {
    return {
      keySpecs: Object.entries(data._keySpecs || {}).map(([k, v]) => ({ id: uid(), label: k, value: String(v) })),
      keyFeatures: Object.entries(data._keyFeatures || {}).map(([k, v]) => ({ id: uid(), label: k, value: String(v) })),
      specGroups: Object.entries(data._specGroups || {}).map(([name, fields]: [string, any]) => ({
        id: uid(),
        name,
        fields: Object.entries(fields || {}).map(([k, v]) => ({ id: uid(), label: k, value: String(v) })),
      })),
    };
  }
  ...
  return { keySpecs: [], keyFeatures: legacyKeyFeatures, specGroups: legacySpecGroups };
}
    ...
    productApi.getById(productId).then(({ data }) => {
      if (data.success) {
        ...
        const existingContent: any[] = p.content
          ? (typeof p.content === "string" ? (() => { try { return JSON.parse(p.content); } catch { return []; } })() : p.content)
          : [];
        setForm({
          name: p.name || "",
          sku: p.sku || "",
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1.5">
          ...
          <Input value={value.productCode} onChange={(e) => onChange({ productCode: e.target.value })} />
        </div>
        <div className="space-y-1.5">
          <Label>Brand</Label>
          <Input
